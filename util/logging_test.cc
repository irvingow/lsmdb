...
TEST(Logging, NumberToString) {
  ...
  ASSERT_EQ("199", NumberToString(199));
...
TEST(Logging, ConsumeDecimalNumberRoundtripTest) {
...
TEST(Logging, ConsumeDecimalNumberRoundtripTestWithPadding) {
  ConsumeDecimalNumberRoundtripTest(0, " ");
  ...
  for (uint64_t i = 0; i < 100; ++i) {
    uint64_t large_number = std::numeric_limits<uint64_t >::max() - i;
    ConsumeDecimalNumberRoundtripTest(large_number, "padding");
  }
}
...
// to be accurate, input_string's leading character is not digit.
