...
class Cache {
 public:
  ...
  /// must call this->Release(handle) when the returned mapping is
  /// no longer needed.
  //
  ...
  virtual Handle* Insert(
      ...
      std::function<void(const Slice& key, void* value)> deleter) = 0;
  ...
  // Return the value encapsulated in a handle returned by
  // a successful Lookup().
  /// REQUIRES: handle must not have been released yet.
  /// REQUIRES: handle must have been returned by a method on *this.
  ...
  // are sharing the same cache to partition the key space. Typically
  // the client will allocate a new id at startup and prepend the
  // id to its cache keys.
  virtual uint64_t NewId() = 0;

  // Remove all cache entries that are not actively in use.
  // Memory-constrained applications may wish to call this
  // method to reduce memory usage.
  // Default implementation of Prune() does nothing. Subclasses
  // are strongly encouraged to override the default implementation.
  virtual void Prune() = 0;

  // Return an estimate of the combined charges of all elements
  // stored in the cache.
